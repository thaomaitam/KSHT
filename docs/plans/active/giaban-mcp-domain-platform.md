...
- Cloudflare Free always-on: Worker `ksht-mcp` at `https://ksht-mcp.ngthanhhuy951.workers.dev/mcp` plus SQLite Durable Object `GiabanShop` (one owner instance). The DO serializes requests and coordinates recovery while business persistence targets the existing live shop KV binding `DB`; this is not a Pages, `ksht-api`, `/api/v1`, or D1 cutover.
- `.mcp.json` points at that URL with header `KSHT_API_KEY=${KSHT_API_KEY}`. Secret is not in git. A read-only Wrangler check confirmed the `KSHT_API_KEY` secret name exists on `ksht-mcp`; its value was not inspected or changed.
- On explicit owner authorization, Worker version `489c8c8e-f442-4439-aa6e-56705b80a36e` was deployed with `GIABAN_SHOP` and the existing live shop KV binding `DB`. `LiveKvStore` performs read-only initial legacy hydration, explicit ID/status/cost mapping, private canonical state on first accepted mutation, changed-document projection, strict public allowlists, same-key throttling, and DO roll-forward journaling.
...
- Tests `tests/mcp/live-kv.test.ts` prove current KV hydration, storefront-compatible product publication without unknown/cost fields, order/payment publication, scoped migration blocking, missing/duplicate-ID blocking, external-writer drift blocking, stale-read rollback prevention, and same-instance partial-write recovery. Before deployment, `npx wrangler deploy --dry-run --config=wrangler.mcp.jsonc` confirmed `GIABAN_SHOP` and live `DB` bindings without reading or changing live data.
- Post-deploy read-only preflight returned `authoritativeStore: kv`, `migrationReady: false`, and `migrationBlockerCount: 697`. Version `663c2845-5665-4c31-91b2-261e7cfb0c2f` enabled `MCP_WRITE_DISABLED=1`; a mutation-shaped probe was rejected at the MCP guard before application/KV execution. Version `45e9aab9-16a0-412d-afbd-763eff1af528` added PII-safe blocker summaries while retaining that kill switch. Live status now reports `mcpMutationsEnabled: false` and capabilities report `mcpWrite: true`. No customer records were exposed and no live KV mutation occurred.
- PII-safe diagnostics group the 697 blockers as 596 `order:customer_id_requires_review` and 101 `order:legacy_total_or_debt_requires_review`. Counts can overlap on one order; they do not establish 697 distinct records. Local evidence: targeted MCP tests passed 20/20, full platform passed 67/67, contract tests passed 4/4, typecheck, `git diff --check`, and Wrangler dry-run passed.

### Live KV reconciliation plan — 2026-09-04

**Objective:** deterministically repair only the legacy order/customer references and historical total/debt semantics needed for `migrationReady: true`, without exposing PII through MCP output, changing storefront documents unnecessarily, or enabling ordinary MCP writes during the process.

**Confirmed baseline:** the write kill switch is active (`mcpMutationsEnabled: false`); 596 blockers require customer-link review and 101 require total/debt review. Live R1 class counts (deploy `1afe1099`) now split those populations; see R1. Existing `scripts/migration/` tooling proves synthetic transforms/count reconciliation only; it is not authorized or safe for direct live repair.

**Non-goals:** no D1 or `/api/v1` cutover, Pages deployment, product/category rewrite, backup/restore MCP exposure, name-only customer matching, concurrent web-admin/MCP writes, or silent rewriting of unexplained historical money.

1. **R1 — Refine read-only aggregate diagnostics.** Add deterministic in-Worker analysis that returns only source hash and counts for: explicit valid customer ID; unique normalized phone plus matching normalized name; unique phone with name disagreement; duplicate-phone ambiguity; no candidate; missing phone; debt with matching computed total; total differing exactly by legacy debt; and unexplained total mismatch. Never return IDs, names, phones, addresses, order lines, amounts, or per-record output. Gate: category counts sum to the relevant blocker populations and synthetic privacy tests prove fixture identifiers/PII cannot appear.
   - Source implemented: `getStatus.migrationDiagnostics` (`sourceHash` SHA-256 of the ten raw legacy KV documents; `customerLinks` and `money` integer counts only). Application sanitizes the payload before return. Contract: `MigrationDiagnostics` on `StatusView`.
   - Local evidence: `tests/mcp/live-kv.test.ts` classification + PII-exclusion; `tests/contract/openapi.test.ts`; `npx tsc --noEmit`; `npm run test:platform` 82/82.
   - Named `ksht-mcp` deploy 2026-09-04: version `1afe1099-70f7-4604-8797-306d2705bc54` via `wrangler.mcp.jsonc`. No live KV writes. `MCP_WRITE_DISABLED=1` is now live (`mcpMutationsEnabled: false`).
   - Live `getStatus` after that version: `migrationReady: false`, 697 blockers (596 customer-id, 101 debt/total). `customerLinks`: explicitValidId 0, uniquePhoneAndMatchingName 357, uniquePhoneNameDisagreement 4, duplicatePhoneAmbiguity 29, noCandidate 3, missingPhone 203 (review classes sum 596). `money`: debtMatchingComputedTotal 2, totalDiffersByLegacyDebt 7, unexplainedTotalMismatch 92 (sum 101). `sourceHash` is the 64-hex live document hash; do not treat it as a credential.
   - Owner-wide Pi client is `~/.pi/agent/mcp.json` server `KSHT`; project `.mcp.json` is optional and must not be required.
2. **R2 — Approve reconciliation policy.** Owner-approved 2026-09-04 (implement the recommended default; no live apply in this slice). Auto-link only an already-valid ID or a unique normalized phone **and** matching normalized name; phone remains a migration candidate, not a permanent identity key. Quarantine all ambiguous/no-candidate cases. For money, derive a historical payment only when computed order total equals the legacy total and `0 <= debt <= total`; treat `total = computed total + prior debt` as prior-debt carryover that must not enter the order total; quarantine every other mismatch. Do not generate or apply a live patch until R4/R5 are separately authorized.
3. **R3 — Build and rehearse a maintenance-only preview/confirm path.** Extend the contract with bounded `previewLiveReconciliation` and `confirmLiveReconciliation` operations. Preview returns only plan hash, source hash, aggregate counts, affected-document names, and blockers. Confirmation must be actor/payload/source-hash/revision-bound, expiring, single-use, and separately gated so ordinary MCP writes remain disabled (`MCP_WRITE_DISABLED=1` stays on; confirm also requires `MCP_RECONCILE_ENABLED=1`, which is unset in `wrangler.mcp.jsonc`). Rehearse against synthetic snapshots, including duplicate phones, changed source between preview/confirm, partial KV failure, retry, and roll-forward. Do not deploy this path and do not set `MCP_RECONCILE_ENABLED` without a named R5 request.
   - Source implemented 2026-09-04: OpenAPI `/migrations/live-reconciliation/{preview,confirm}`; registry ops + `migration:reconcile` scope; `LiveKvStore.previewLiveReconciliation` / `applyLiveReconciliation` patches **orders only** (auto-link unique phone+matching name; `paidAmount` for debt-matching totals; strip prior-debt carryover). Quarantined classes remain blockers (`migrationReady` stays false). Confirm is skipped by `runInTransaction` blockedKeys and uses the DO journal. MCP: preview allowed under `MCP_WRITE_DISABLED=1`; confirm also needs `MCP_RECONCILE_ENABLED=1`; ordinary writes stay killed.
   - Local evidence: `tests/mcp/live-kv.test.ts` (counts, PII denylist, apply, source drift, reused token, MemoryStore fail-closed, journal roll-forward); `tests/mcp/mcp.test.ts` kill-switch; `tests/contract/openapi.test.ts`; `npx tsc --noEmit`; `npm run test:platform` 87/87. Skipped `test:worker` / `npm run build`. R3 source was later deployed under named R5 authority; see R5.
4. **R4 — Prepare recovery and freeze.** Under separate explicit authority, stop legacy web-admin writes, take a checksum-verified encrypted snapshot of all ten legacy KV documents outside git/log/chat, record retention and recovery location, and prove restore in an isolated namespace. If the source hash differs from R1/preview, discard the plan and restart diagnostics.
   - Owner authorized 2026-09-04. Freeze is operational only: do not use web-admin; `ksht-api` was not redeployed. Encrypted AES-256-GCM snapshot of the ten live KV documents is on this device outside git/chat (`/root/.pi/agent/backups/ksht-kv-2026-09-04T09-28-56-159Z`). Restore rehearsal: local decrypt+auth+sha256 of all ten documents, no Cloudflare namespace clone (avoids duplicating PII). `sourceHash` matches R1: `6d3067c1198df469b82fba2cc3448a0d8040624b3e704b3fdfbfd2d7471c1497`. Key file is local 0600; do not print, commit, or log it.
5. **R5 — Apply the narrow live reconciliation.** Under separate exact authorization, enable only the maintenance confirmation path, keep ordinary writes killed, and let the singleton DO journal the changed `orders`/`customers` documents plus canonical committed state. Preserve existing explicit IDs and untouched fields. Any ambiguous case, source drift, count mismatch, KV 429, or partial publish fails closed and rolls forward from the journal rather than starting a second repair.
   - Owner authorized 2026-09-04. `wrangler.mcp.jsonc` now has `MCP_RECONCILE_ENABLED=1` and still `MCP_WRITE_DISABLED=1`. First R5 deploy `7a7aac80-14d6-4d4a-be84-d12915c15b26` could preview but confirm failed closed (`INTERNAL_ERROR`, no KV change) because SQLite Durable Object `storage.put` rejects values over 2MB and the unchunked journal held the full `orders` envelope. Source fix: chunk DO pending/committed blobs at 900_000 chars. Redeploy `b52585b4-9913-4131-b6c8-27ffdcd88942`.
   - Live apply: `previewLiveReconciliation` then `confirmLiveReconciliation` against pre-apply `sourceHash` `6d3067c1198df469b82fba2cc3448a0d8040624b3e704b3fdfbfd2d7471c1497`, `planHash` `7e2f1e25da8cc446ab0ca391fcfe5b3924d64078650ba2df53145f79f64c4fda`. Counts: auto-link 357, quarantine customer 239, historical payment 2, strip prior-debt 7, quarantine money 92, preserve explicit 0. Affected document: `orders` only.
   - Post-apply `getStatus`: `mcpMutationsEnabled: false`, `migrationReady: false`, 331 blockers (239 `order:customer_id_requires_review`, 92 `order:legacy_total_or_debt_requires_review`). Diagnostics: explicitValidId 357, uniquePhoneAndMatchingName 0, uniquePhoneNameDisagreement 4, duplicatePhoneAmbiguity 29, noCandidate 3, missingPhone 203; money unexplainedTotalMismatch 92, other money classes 0. New `sourceHash` `ffbb356890122273459f2aa8bdc8addbd16632eac13a216e4cf261005107a4c1`. Per-document SHA-256 vs R4 snapshot: only `orders` changed; products/categories/settings/customers/costPrices/transactions/bankInfo/taxRate/shopTemplates unchanged. Capabilities: `mcpWrite` kill on, `mcpReconcile` kill off. Ordinary MCP writes remain disabled. R6 (clear remaining quarantines / enable writes) is not authorized.
6. **R6 — Verify before enablement.** Require `migrationReady: true`, blocker count zero, unchanged product/category/settings hashes, expected order/customer counts, reconciled monetary aggregates, storefront read success, and no secret/PII in diagnostics or logs. Keep the write kill switch on through an observation window. Removing `MCP_WRITE_DISABLED` is a separate deploy and requires explicit authorization; enable one low-risk real workflow before financial operations.
   - Remaining-quarantine review (offline, 2026-09-04): `buildQuarantineReview` + `npm run migration:quarantine:preview` emit PII-safe integer aggregates only (name/diacritic buckets, duplicate-phone groups, money sign buckets, overlap). No IDs/names/phones/amounts. Does not patch live KV.
   - Owner policy 2026-09-04: do **not** repair remaining historical order-creation rows (331 quarantines). Leave them in place; do not auto-link, rewrite money, or invent phones. Future operations are the goal. Deleting that historical order-creation set is an optional later named action (preview/confirm), not authorized now.
7. **R7 — Grandfather historical reviews and enable ordinary MCP writes.** Named owner request: keep 331 historical review rows unrepaired and reported; stop document-fencing `orders`/`customers` for `customer_id_requires_review` and `legacy_total_or_debt_requires_review`; preserve those rows on publish (no synthetic `legacy_customer_*`); set `MCP_WRITE_DISABLED=0` on `ksht-mcp`. `migrationReady` stays false. Do not delete history. Do not Pages/D1/`ksht-api` cutover. Do not use web-admin concurrently.
   - Deployed `ksht-mcp` `f5cd293a-9e24-4e7a-baf3-1a89d73d6638`. Live evidence: `MCP_WRITE_DISABLED=0`, `mcpMutationsEnabled` and `killSwitches.mcpWrite` from getStatus/getCapabilities after deploy. Historical blocker count remains 331. No live order/customer mutation in this step.
...
### Catalog live apply — owner-authorized 2026-09-04

Owner confirmed: (1) the 13 dry-run groups are the live taxonomy; (2) web-admin catalog edits stay frozen during apply; (3) named live actions: off-git checksummed copy of the ten KV keys, catalog-only apply of names/categories while preserving ids/prices/variants/images, and a temporary `ksht-mcp` deploy with `MCP_WRITE_DISABLED=0` then restore to `1`.

- Apply is catalog-only. Do not write orders, customers, payments, settings, bank, or tax. Do not reconcile the 697 blockers in this slice. Do not deploy Pages, enable `DOMAIN_AUTHORITATIVE=1`, or cut over D1.
- Abort if live classification fingerprint drifts from `ac0d1649b83c84cd1510cea79c1270d76e210b302186aaab32135e45dbe1b785`.
- Recapture live products with integer variant prices before any MCP catalog write. Do not apply the classification-only snapshot.
- Recovery: restore the ten KV documents from the off-git backup only with a later named command; Wrangler rollback restores Worker code, not KV data.
...
- Personal MCP access is private owner-only through the `KSHT_API_KEY` Worker secret and matching Pi client environment; GitHub OAuth is not part of this path.
- Cloudflare KV remains the live business source of truth for the deployed personal MCP path. The Durable Object serializes requests and keeps recovery/committed state; production D1 and `/api/v1` cutover remain unperformed. Browser local storage is never an admin write source.
...
- The deployed personal MCP uses a dedicated API-key-authenticated Worker, one `GiabanShop` Durable Object, and direct binding to the existing shop KV namespace. The separate Domain/API Worker and D1 design remains source-only and requires an explicit future cutover.
...
- The named `ksht-mcp` deployment and read-only preflight were explicitly authorized and completed. Any live data mutation, reconciliation, production Domain/D1 cutover, Pages publication, or other Cloudflare change still requires exact action-specific authorization.
