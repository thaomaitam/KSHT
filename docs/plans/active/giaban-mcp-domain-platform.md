...
**Confirmed baseline:** the write kill switch is active; 596 blockers require customer-link review and 101 require total/debt review. The current aggregate counts do not reveal overlap, candidate-match quality, or whether a total mismatch is explained by legacy debt. Existing `scripts/migration/` tooling proves synthetic transforms/count reconciliation only; it is not authorized or safe for direct live repair.
